 ...
 * with only a light-touch link to the full progress dashboard. */
export default function HomeStats({ onStartTopic }: Props) {
  ...
  return (
    <div style={{ marginTop: 18, display: "flex", flexDirection: "column", gap: 12, alignItems: "center" }}>
      {/* Today's spark */}
      <button
        type="button"
        onClick={() => {
      ...
      >
        ...
        <span style={{ color: "var(--text-tertiary)", fontWeight: 500 }}>Today&apos;s spark:</span>
        <span style={{ fontWeight: 700, color: "var(--accent)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
          {topic}
        </span>
        <span aria-hidden="true" style={{ fontSize: 14, color: "var(--text-tertiary)" }}>→</span>
      </button>
      ...
      {inProgress && (
        ...
        >
          ...
          <span style={{ flex: 1, minWidth: 0 }}>
            <span style={{ display: "block", fontSize: 11, color: "var(--text-tertiary)", fontWeight: 600, letterSpacing: "0.04em", textTransform: "uppercase" }}>
      ...
      {hasActivity && (
        ...
        >
          <span className={streak > 0 ? "flame-flicker" : undefined} style={{ filter: streak > 0 ? "none" : "grayscale(1) opacity(0.6)" }}>
            🔥
          </span>
          <span style={{ fontWeight: 700, color: "var(--text-secondary)" }}>{streak}</span>
          <span>·</span>
          <span>
            Level <strong style={{ color: "var(--text-secondary)" }}>{info.level}</strong>
          </span>
