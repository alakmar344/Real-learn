...
- **Optional learning personalization.** After signing in, users can set learning-style preferences (checklist) and free-text notes (500-character cap) in a one-time onboarding modal and in Settings. The data lives only in the device's localStorage, is sent with each lesson request, and is injected into the LLM prompt so explanations can be tailored; the backend validates and caps it but never stores it. Privacy Policy v2.7, Terms of Service v2.5, and Cookie Policy v2.3 introduce the disclosure and re-prompt all users for consent.
...
> **Note:** A Google AI Studio connectivity test was attempted from Render and returned `FAILED_PRECONDITION / "User location is not supported for the API use."`. Google's API rejects requests from Render's region, so no Google AI Studio diagnostic is wired into the backend.
