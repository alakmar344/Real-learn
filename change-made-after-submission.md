...
**Today — July 25, 2026**
- **Gen Z & Gen Alpha UI/UX Transformation:** Overhauled reading surfaces and interactive components to align with Gen Z and Gen Alpha learning preferences. Introduced TL;DR concept banners at the top of lesson parts (`PartCard.tsx`), live visual animated audio waveform equalizers on active speech synthesis (`ListenButton.tsx`), spring-physics hover/active kinetic feedback on quiz choice cards (`QuizQuestion.tsx`), and kinetic card utilities in `globals.css` (maintaining strict compliance with Rule 4 Soft Pastel system without purple/violet).
...
| 2026-07-24 | backend | Wire Google AI Studio startup ping into `src/server.js`: on server listen, fire-and-forget `runGoogleAiStudioPing()` if `GOOGLE_AI_STUDIO_API_KEY` is set. Keeps self-contained env-only behavior, logs start/status/error, never blocks or fails startup. Updated docs (`README.md`, `AGENT_MEMORY.md`, `change-made-after-submission.md`). |
| 2026-07-24 | revert | Remove Google AI Studio diagnostic ping after Render test returned `FAILED_PRECONDITION / "User location is not supported for the API use."` — Google blocks API access from Render's region. Removed `backend/src/scripts/ping-google-ai-studio.js`, its `ping:google-ai-studio` npm script, `GOOGLE_AI_STUDIO_API_KEY` from `.env.example`, and the startup hook in `src/server.js`. Updated docs (`AGENT_MEMORY.md`, `change-made-after-submission.md`, `README.md`). |
