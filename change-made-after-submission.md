...
- **Re-Add Fast/Explain mode chosing pill on the /learn topbar (UX):** The mode selector was removed from the lesson topbar on 2026-08-16 during the header-declutter pass (leaving it only on the homepage and Settings). Reinstated it as an **interactive "chosing" pill** — not the old read-only FAST/EXPLAIN badge — using the shared `LESSON_MODES` constant and the sliding `.mode-glider` now given a compact topbar variant (`.mode-glider--compact`, `.learn-topbar__mode`). The pill reads/writes the persisted preference store (`mode`/`setMode`), so fiddling with it mid-lesson steers the next question just like the homepage toggle. `flex-shrink: 0` + `min-width:0` on the question keep the original narrow-screen width concern in check (the question ellipsizes rather than being pushed out).
