...
- 2026-08-05 — **Night Owl (Swiss Industrial Brutalism) hero component + showcase route.** New self-contained `components/homepage/NightOwlHero.tsx` built with Tailwind utility classes, for a proposed brutalist direction: monospace spec-bar navbar (REALLEARN mark + STREAK // 19 + LVL // 14 + theme switch), heavy all-caps hero "NIGHT OWL MODE, PRO" with the "PRO" as a transparent fill + 1.5px foreground `-webkit-text-stroke` outline, a 1px-bordered input card (`SYS_PROMPT // DIRECT` spec header, live 0000 / 1000 padded char counter, "ASK LITERALLY ANYTHING." textarea, TL;DR / DEEP DIVE radiogroup with solid-inverted active states and thin-bordered inactive states, a clickable `TRY:` sample bar with a `[CLR]` button), and a full-width inverted "TEACH ME →" CTA with `active:scale-[0.99]`. Zero glow / gradient / shadow; two strict themes (deep carbon night + off-white stone day) scoped through `--no-*` tokens in `globals.css` (`.night-owl--night` / `.night-owl--day` / `.night-owl__outline`, reduced-motion gated). Demo at `/night-owl` (excluded from the app sidebar via `HIDE_SIDEBAR_PREFIXES`); its CTA is wired to the real lesson pipeline (`useLesson().generateLesson` + preference store mode). Verified: `tsc --noEmit` clean, `next lint` clean, `next build` clean, `verify:quiz` + `verify:achievements` pass.
