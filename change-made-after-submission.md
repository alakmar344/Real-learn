...
- **First-Question Reliability & Cold-Start Generation Failure Root Cause Fix:** Fixed the chronic issue where the first question asked after opening RealLearn failed. Root cause was twofold: (1) Render cold boot (~30-45s) caused gateway 502/503/504 errors that exhausted the previous 3-attempt (6s total) retry budget before the server booted; (2) Clerk session token resolution on initial cold mount could return null or un-retryable 401s; (3) Provider first-byte timeout was too aggressive (20s) for initial model warmup. Fixed by: (a) adding proactive non-blocking `warmupBackend()` pings on page load and textarea focus; (b) extending `useLesson.ts` to 5 retry attempts with exponential backoff covering the full ~45s boot window; (c) adding cold-token resolution grace period and 401 recovery; (d) raising `gemma.js` first-byte timeout to 35s; (e) adding `www.reallearn.site` and local origins to default allowed origins.
