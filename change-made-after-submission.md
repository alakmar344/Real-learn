...
- **Security Hardening: Strict Authorized Party (AZP) Validation & Elimination of Wildcard Vercel Allowance:** Hardened Clerk token `azp` validation in `backend/src/lib/auth.js`. Completely eliminated wildcard `.endsWith(".vercel.app")` checking which could have allowed tokens from arbitrary Vercel apps. Pinned `AUTHORIZED_PARTIES` strictly to the exact trusted origins (`https://reallearn.site`, `https://www.reallearn.site`, `https://real-learn.vercel.app`) or explicitly configured `CLERK_AUTHORIZED_PARTIES`. Added offensive test `K2` in `backend/test/offensive-audit.test.js` validating that unauthorized origins (including attacker `.vercel.app` domains) are strictly rejected.
