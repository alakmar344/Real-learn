...
| 2026-07-27 | `fix-quiz-answer-validation` | Fix Quiz Answer Validation & Index Alignment: resolved 1-based index off-by-one errors from LLMs by adding `alignQuizCorrectIndex` in backend `validation.js` and `sanitizeQuestion` in frontend `quizShuffle.ts` & `QuizSheet.tsx`. Prompts updated to strictly enforce 0-based indexing. Added backend regression tests in `quizValidation.test.js`. |
