...
**August 19, 2026**
- **Design Richness, Vibrant Colors & Luminous Depth Polish Pass (`globals.css`, `palette.ts`, `achievements.ts`):**
  ...
  - **Dopamine Subject Spectrum**: Saturated subject tokens (Physics, Chemistry, Economics, Biology, CS, History, General) across dark and light modes, and enriched `.part-card__tag--subject` with matching translucent washes, vibrant border strokes, and glowing indicator dots.
...
- 2026-08-19 (later) — **Forensic Security Audit & Bug Remediation Pass.**
  - **CORS & Clerk `azp` Typosquat Whitelist Remediation (`backend/src/middleware/security.js`, `backend/src/lib/auth.js`, `backend/.env.example`)**:
    - Removed unregistered typosquat domain `"https://reallearn-taupe.xercel.app"` from CORS `allowedOrigins`, Clerk `DEFAULT_PRODUCTION_AUTHORIZED_PARTIES`, and example configuration templates.
  ...
  - **Empirical Verification**:
    ...
    - Frontend: clean ESLint, `tsc --noEmit` typecheck, and Next.js 15 production build (`next build`).
...
- 2026-08-21 — **Deep Reliability, Security & Performance Audit Pass (backend + frontend).**
  - **Backend — startup correctness (`src/server.js`, `src/cluster.js`, `src/bootstrap.js`)**:
    - **Critical**: cluster workers and the bootstrap Node fallback imported `server.js` without ever calling `startServer()` — `isMainModule()` is false for imported modules, so `npm start` without Bun and `npm run start:cluster` came up with **zero listening sockets**. Both entrypoints now invoke `startServer()` explicitly (verified end-to-end: cluster now serves `/api/ready`).
    - Cluster crash-loop backstop: 5 consecutive workers dying within 5s of forking (e.g. fatal config error) now exits the primary instead of hot-loop re-forking forever.
    - Background warm-ups/migrations moved into their own try/catch so a synchronous throw can no longer `process.exit(1)` an already-listening healthy server.
    - **Security**: the boot-time `curl -fsSL https://bun.sh/install | bash` auto-install is now **opt-in only** (`BUN_AUTO_INSTALL=1`) — piping a remote installer into bash on every cold start was a supply-chain RCE surface with all provider/DB secrets in env.
  - **Backend — rate limiter (`src/lib/rateLimit.js`)**: added `noUpdateTTL: true` to both counter stores. `lru-cache` refreshes TTL on every `set`, and counters were bumped on every request including 429'd ones — a retrying client (or busy shared-NAT IP) slid its window forward indefinitely and never recovered after `windowMs`.
  - **Backend — single-flight stampede fix completion (`src/routes/lesson.js`)**: when the **leader's** client disconnected mid-generation, `finishRequest` aborted the generation and rejected the shared in-flight promise — failing **every follower** in exactly the shared-classroom-link scenario the single-flight guard exists for. The leader now *detaches*: response-side teardown is split from generation-side teardown, follower connections are counted per cacheKey, and a leader disconnect with live followers lets the generation finish, pass moderation, populate the cache, and resolve all followers. Still bounded by `LESSON_TIMEOUT_MS`.
  - **Backend — hot-path efficiency (`src/routes/lesson.js`)**: personalization adaptation-plan parsing (regex splits + directive ranking) moved after the cache / follower / concurrency gates — cache hits and followers no longer pay for a plan only generation leaders use.
  - **Backend — Serper lifecycle (`src/lib/serper.js`)**: `fetchRealWorldContext` accepts the lesson's abort signal (`AbortSignal.any` with its own 4s timeout) so client disconnects no longer leave orphaned outbound sockets.
  - **Backend — TTS (`src/routes/tts.js`)**: single-flight dedup for identical concurrent syntheses (N same-text requests share one Edge-TTS run instead of burning N of 8 slots); boot-time temp-dir cleanup only removes files older than 1h so a rolling deploy can't delete another live instance's in-progress file (ENOENT 500).
  - **Frontend — cross-account isolation (`lib/userScopedStorage.ts`)**: **critical** — in-memory stores are now reset on ANY transition leaving a signed-in scope, including direct `user_A → user_B` switches (Clerk multi-session). Previously only `→ anon` reset, so user B with empty scoped storage saw user A's XP/streak/journeys/personalization and persisted them under B's keys on the next write. New regression assertion in `scripts/verify-user-scoping.mjs`.
  - **Frontend — durability (`lib/debouncedStorage.ts`)**: a `localStorage.setItem` throw (quota / private mode) no longer silently drops the pending state; it stays queued and retries on the next debounce tick / pagehide / scope-switch flush.
  - **Frontend — capability discovery (`lib/api.ts`)**: `fetchReady` memoizes only successful probes; one cold-start timeout used to cache `null` and disable capability discovery for the entire session.
  - **Frontend — share cards (`components/learning/ShareResult.tsx`)**: canvas now resolves the real `next/font` family names from `--font-inter` / `--font-display` CSS vars and awaits `document.fonts.ready` — every share PNG had been silently rendering in the browser's default sans-serif instead of the brand faces.
  - **Frontend — rendering (`LoadingCinematic.tsx`, `AchievementsGrid.tsx`, `ScrollToTop.tsx`)**: progress interval created once and fed by refs (each SSE progress event previously destroyed/rebuilt it and could freeze the bar); per-badge earned/progress derivation memoized (whole `progress()` set re-ran per hover); scheduled rAF cancelled on unmount.
  - **Frontend — network/bundle (`app/layout.tsx`, `lib/ttsCache.ts`, `hooks/useSpeech.ts`, `app/settings/page.tsx`)**: dropped the unconditional `<link rel="prefetch" href="/learn">` (auth-gated route → wasted redirect fetch on every signed-out first paint); TTS blob LRU extracted to `lib/ttsCache.ts` so the settings page's cache-clear no longer bundles the entire speech implementation (`lru-cache` + `remove-markdown` + Edge-TTS plumbing).
  - **Verification**: backend `node --test` **116/116** green; cluster + bootstrap smoke-tested end-to-end (serve `/api/ready`, crash-loop backstop exits after 5 fast deaths); frontend `tsc --noEmit` clean, ESLint clean, all 9 verify scripts pass, `next build` green.

- 2026-08-21 (later) — **Bun promoted to primary runtime with a pinned, checksum-verified installer (`backend/src/bootstrap.js`).**
  - **Bun is the main runtime again**: `npm start` (bootstrap) auto-installs Bun by default when it's missing, instead of requiring the `BUN_AUTO_INSTALL=1` opt-in from the earlier hardening pass. Node.js 24 remains the automatic secondary runtime whenever Bun can't be acquired or verified.
  - **Supply-chain hardening kept — done properly this time**: the `curl -fsSL https://bun.sh/install | bash` remote-script install is GONE. The bootstrapper now downloads the **version-pinned** release archive (`bun-v1.4.0`) straight from GitHub releases, verifies its **SHA-256 against checksums baked into the source** (linux x64 / x64-baseline / aarch64), refuses to install on any mismatch, unzips, `--version` smoke-tests the binary (falling back to the baseline build on pre-AVX2 CPUs), and caches the install under `~/.bun-pinned/v<ver>/` for instant reuse on later boots. `BUN_AUTO_INSTALL=0` disables the auto-install (Node fallback).
  - **Verification** (all live, end-to-end): fresh boot downloads + verifies + serves `/api/ready` under Bun; second boot reuses the cached pinned install with no download; `BUN_AUTO_INSTALL=0` boot serves under the Node fallback; backend `node --test` 116/116 green.

- 2026-08-21 (later still) — **Encoding-hardening & caching pass (backend + frontend).**
  - **Lesson cache key made injective (`backend/src/lib/lessonCache.js`)**: key material was a `"|"`-join of user-controlled fields (question, notes/goals, learningContext) that can themselves contain `"|"` — e.g. personalization notes `"a"` and learningContext `"a||"` produced byte-identical material, so two semantically different requests shared one cached lesson (a cross-cohort cache-poisoning primitive). Components (and the personalization sub-encoding, including checklist arrays that were ambiguous under `","`-join) are now serialized as JSON arrays — delimiter-free, unforgeable boundaries. Deploy rotates cache keys once; the cache re-warms. New regression test `G4` in `offensive-audit.test.js` pins the exact historical collision pairs.
  - **JSON-LD script hardening (`frontend/app/layout.tsx`)**: all four structured-data blocks now emit through a `jsonLd()` helper that escapes `<` as `\u003c` — no string value can ever terminate the `<script>` block early (`</script>` breakout). Content is static today; this keeps it safe if any field becomes dynamic.
  - **Personal-data export (`backend/src/routes/account.js`)**: `/api/export-data` now sends `Cache-Control: private, no-store` — the user's complete stored personal data must never sit in a shared/proxy/disk cache.
  - **Static-asset caching (`frontend/next.config.js`)**: `*.png` (og-image, PWA icons) get `max-age=86400` + SWR, and `manifest.json`/`llms.txt` get `max-age=3600` + SWR — social-preview scrapes, PWA launches, and crawler visits stop re-downloading them at full cost.
  - **Verification**: backend `node --test` **117/117** (including new G4); frontend `tsc --noEmit` clean, ESLint clean, `next build` green.

- 2026-08-21 (final) — **Surgical latency / reliability / UX pass (backend + frontend), from a full audit — no rewrite.**
  - **Backend — moderation logging off the request path (`src/routes/lesson.js`, `src/startup/migrations.js`, `src/lib/moderationLog.js`)**: the three `await logModerationEvent(...)` calls on blocked input / fast-output / explain-output are now fire-and-forget (`void`), so a flagged user gets their 400/error immediately instead of waiting on a Mongo write. The **first** flagged request no longer pays for the moderationLogs TTL `createIndex` + legacy `updateMany` backfill inline either — `ensureModerationLogTtlIndex` is now exported and built once at startup inside `ensureUserDataIndexes`.
  - **Backend — /health ping amplification (`src/routes/health.js`)**: the MongoDB `ping` result is cached for 3s, so a distributed burst of health checks (past the per-IP limiter, many IPs) collapses onto one ping per window instead of one DB command per hit. The in-memory AI snapshot stays fresh.
  - **Backend — bounded single-flight followers (`src/routes/lesson.js`, `src/config.js`)**: followers on one hot cacheKey are capped (new `MAX_IN_FLIGHT_FOLLOWERS_PER_KEY`, default 200); beyond the cap the server sheds load with a retryable 503 instead of opening yet another SSE connection + heartbeat/progress/slow-notice timer set for up to the lesson deadline.
  - **Frontend — quiz gate integrity (`app/learn/page.tsx`, `components/learning/Flashcards.tsx`)**: the Flashcards deck (built from EVERY part's content) was mounting mid-lesson, leaking excerpts of still-locked parts past the quiz gate and cluttering the page. It now renders only at completion, where recall practice belongs.
  - **Frontend — the payoff is no longer off-screen (`app/learn/page.tsx`)**: acing the FINAL quiz now smooth-scrolls the completion screen into view (respecting `prefers-reduced-motion`); previously the celebratory screen mounted below the fold and the viewport stayed parked on the last part card.
  - **Frontend — no dead end after fast-mode completion (`components/learning/CompletionScreen.tsx`)**: "Go deeper" follow-ups were gated on `keyTakeaways`, so fast-mode / salvaged lessons (no takeaways) lost all post-completion CTAs. They now gate on the actual (topic-derived, memoized) suggestion list, which is never empty.
  - **Frontend — export-data feedback (`app/settings/page.tsx`)**: "Export my data" now has an `exporting` pending state (disabled + "Exporting…"), mirroring Delete, so a slow backend export no longer feels dead and double-clicks can't fire multiple downloads.
...
- 2026-08-22 — **Zero-friction rebuild: "open → ask → understand" for zero-tech users (frontend + backend).**
  - **Answers rebuilt around doubt-killing clarity (`backend/src/lib/prompts.js`)**: the Gen-Z-slang tutor voice (itself jargon for most people) is replaced with a plain-language, warm voice for everyone from teens to grandparents — every technical term defined the moment it appears, every answer covering the what / why / how / connections with universal daily-life analogies, and the reader's most likely next doubt anticipated and answered in the text. Part intents renamed: Foundation → **The big idea**, Mechanism → **How it works**, Real World → **Where you see it**. JSON schema, word counts, and validation contract unchanged; backend 117/117 tests green.
  - **No forced onboarding (`components/onboarding/OnboardingRedirect.tsx`)**: brand-new visitors are no longer bounced into the 5-step wizard before seeing the product — they land directly on the ask box, exactly like ChatGPT. The single consent + age dialog (`PreSignInConsent`) appears once at sign-in, the moment it actually matters. The wizard route still works for direct visits.
  - **Nothing is locked anymore (`components/learning/PartCard.tsx`, `app/learn/page.tsx`, `components/learning/ProgressRail.tsx`)**: removed the lock veil, the 12px content blur, and the "Pass the Part N-1 check to unlock" wall — every part of every lesson is readable top to bottom immediately, like an article. The reading-progress bar stays as ambient feedback.
  - **Quizzes are now optional self-checks (`components/learning/QuizSheet.tsx`)**: the 100%-score-to-advance gate and forced retry loop are gone. Any completed attempt continues ("Continue →"); retrying missed questions (banked answers, reshuffled options) is offered as a choice ("Or try the missed ones again"). First-attempt scores still power perfect-run stats/achievements.
  - **Jargon sweep across the UI**: "Foundation/Mechanism/Real World" → "The big idea / How it works / Where you see it" (rail, loading cinematic, consent modal, meta); "Journey(s)" → "Lesson(s)" (toasts, completion screen, progress stats); mode labels "Fast/Explain" → "Quick answer / Full lesson" (`lib/lessonModes.ts`); level labels "Class 6-8 / 9-10 / College" → "Simple / Standard / Advanced" with plain hints (`LevelSelector.tsx` — backend enum values untouched); homepage how-it-works strip rewritten in plain words (`HomeStats.tsx`).
  - **Docs updated**: README, `llms.txt`, `llms-full.txt`, `docs/AGENT_MEMORY.md` §1 reflect the new product model.
  - **Verification**: backend `node --test` **117/117** green; frontend `tsc --noEmit` clean, ESLint clean, `next build` green.
