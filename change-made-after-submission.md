...
| 2026-07-24 | scripts | Add `backend/src/scripts/ping-google-ai-studio.js` — one-off Node script that pings Google AI Studio with a “hi” prompt for post-deploy diagnostic logging only; never imported by the server. Added `GOOGLE_AI_STUDIO_API_KEY` to `.env.example` and `ping:google` npm script. Updated docs (`AGENT_MEMORY.md`, `change-made-after-submission.md`). |
