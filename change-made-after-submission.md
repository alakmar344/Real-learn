...
| 2026-07-23 | backend hardening | Backend indexing, search, caching, and test passes: added compound text index on `lessonCache` (`lesson.title`, `lesson.keyTakeaways`, `lesson.content`) and `GET /api/search-lessons` for authenticated lesson search; ensured per-entry TTL + unique key indexes on `lessonCache`; enabled `Vary: Accept-Encoding` globally plus `Cache-Control` on all GET endpoints (`/api/auth-debug`, `/api/export-data`, `/api/search-lessons`); exported `app` from `server.js` and gated server auto-start behind `SKIP_SERVER_START=true` so tests can import the app; added `backend/test/search.test.js` and `backend/loadtest.js`; all 27 backend tests pass. |
