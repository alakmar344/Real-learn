...
quiz you must score 100% on to advance. Lessons are generated by ultra-low-latency
Groq LPU inference (Qwen 3.6 27B / GPT-OSS 120B primary, official groq-sdk), backed by
NVIDIA NIM high-capacity 70B–150B parameter models (Llama 3.3 70B, Mistral Large 123B,
Nemotron 70B, Qwen 2.5 72B fallback), and Cloudflare Workers AI (Llama 3.3 70B FP8 fast last-resort),
grounded with live Serper news in Part 3, streamed over SSE, and cached in a two-tier
LRU + MongoDB cache. The aesthetic is the **Olive Frenzy Minimal** design language:
...
│       ├── lib/gemma.js       AI engine (Groq primary, NVIDIA NIM 70B-150B fallback, Cloudflare Workers AI last-resort, retries, circuit breaker)
