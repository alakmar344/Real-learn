...
- 2026-08-14 — **Chatbox UI Modernization, Mode Simplification & Clutter Removal**:
  ...
  - **Chat Box Redesign & Consistent Enter Action**: Redesigned `QuestionInput.tsx` into a lightweight, modern single-bar layout (`.q-form__bar`), moving mode glider and tools into unified left/right groupings. Enabled natural `Enter` key form submission (with `Shift+Enter` for multiline) and unified polished `Ask →` action buttons.
