...
aesthetic is the **Solar Terracotta** design language (with the "Sunset Pop"
vibrancy pass): solar terracotta `#EE5125` (light) / glowing ember `#FF6435`
(dark) as the ONE interactive accent, sunset gradients (terracotta → rose
light / amber → ember → rose dark, via `--accent-gradient` /
`--text-pop-gradient`) reserved for CTAs and one hero display moment, an
emerald companion (`#04A16C` / `#00D284`) for success/progress, a dopamine
subject-color spectrum for chips, warm alabaster paper `#FAF9F6` by day and
midnight obsidian `#0D1117` by night, plus airy transform-only ambient
auroras (perf-tier gated). Body text is
Inter 400 (bold is for headings/labels only), lesson prose never drops below
16px on phones, every color pairing is WCAG-AA-verified, and there is NO
neon and NO purple/violet (owner's rule). Two moods of the same room: Paper
(day) and Ink (night). JS-side colors (confetti, share card) come from
