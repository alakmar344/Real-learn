...
picks a language (63 global languages), a level (Class 6-8 / 9-10 / College),
...
- Generated lesson text (63 languages) carries `lang` + `dir` attributes via
  `lib/locale.ts` (`contentLangAttrs()`, `dir="rtl"` for Urdu, Arabic, Hebrew, Persian) — apply it to
...
- 2026-08-14 — **Trust Signals & Positioning Simplification Pass**:
  ...
  - **Trust Markers & Kinetic Ticker**: Added a dedicated high-integrity trust strip to `HomeStats.tsx` for new visitors (63 Global Languages, Class 6 to College Level, Live Fact Grounding, Zero Ads & Private) and upgraded `HeroTicker.tsx` with outcomes-based chips (`master concepts`, `3-step clarity`, `active recall`, `curriculum aligned`, `live fact grounding`, `zero ads & private`).
...
- 2026-08-20 — **System Performance Optimization, Reliability Hardening & Architecture Cleanups.**
...
- 2026-08-21 — **Deep audit pass: startup correctness, single-flight completion, cross-account isolation.**
  - **Backend**: cluster workers / bootstrap Node fallback now explicitly call `startServer()` (imported `server.js` never auto-starts — both paths previously came up with zero listening sockets); cluster crash-loop backstop (5 fast worker deaths → exit, not infinite re-fork); warm-ups moved out of the fatal startup catch; `curl | bash` Bun auto-install now opt-in via `BUN_AUTO_INSTALL=1` (supply-chain hardening); rate-limit counters use `noUpdateTTL` (429'd retries no longer extend the window forever); a single-flight **leader disconnect now detaches instead of aborting**, so followers still receive the lesson (counted via `inFlightFollowerCounts`); personalization plan parsing deferred past the cache/follower gates; Serper fetch tied to the lesson abort signal; TTS gained single-flight synthesis dedup + age-gated (1h) temp-file cleanup.
  - **Frontend**: `switchUserScope` resets in-memory stores on ANY exit from a signed-in scope (fixes direct `user_A → user_B` cross-account leak; new assertion in `verify-user-scoping`); debounced storage retries instead of dropping writes on `setItem` throw; `fetchReady` only memoizes successes; share-card canvas resolves real `next/font` families from CSS vars + awaits `document.fonts.ready`; LoadingCinematic's progress interval is created once (refs, no per-event teardown); AchievementsGrid badge derivation memoized; ScrollToTop cancels its rAF; removed the unconditional `/learn` prefetch (auth-gated → wasted redirect); TTS blob LRU extracted to `lib/ttsCache.ts` so settings no longer bundles the speech stack.
  - **Verification**: backend 116/116 tests; cluster + bootstrap smoke-tested (serve `/api/ready`; backstop exits after 5 fast deaths); frontend `tsc` clean, ESLint clean, 9/9 verify scripts, `next build` green.

- 2026-08-21 (later) — **Bun is the primary backend runtime; Node is secondary.** `bootstrap.js` no longer pipes `bun.sh/install` into bash and no longer needs an opt-in: it installs a **pinned Bun release (v1.4.0) verified against baked-in SHA-256 checksums** (x64 / x64-baseline / aarch64), smoke-tests the binary, caches it under `~/.bun-pinned/`, and launches the server under Bun. Any download/verify/run failure falls back to Node 24 automatically; `BUN_AUTO_INSTALL=0` disables the installer. To upgrade Bun: bump `BUN_VERSION` + refresh `BUN_SHA256` from that release's `SHASUMS256.txt`. Verified live: fresh install, cached reuse, and opt-out fallback all serve `/api/ready`.
...
- 2026-08-22 — **Radically Zero-Friction, Beginner-First Pedagogical Upgrade & Unified Linear Onboarding.**
  - **Doubt-Killing Four Pillars AI Architecture (`backend/src/lib/prompts.js`)**: Upgraded `VOICE_AND_SAFETY`, `GENERATE_FAST_ANSWER_PROMPT`, and `GENERATE_LESSON_PROMPT` to enforce casual, conversational everyday English (no niche Gen-Z slang/memes, no stiff formal/academic lecturing) and universal real-life analogies (groceries/buying, cooking, waiting in line, traffic, phone charging, daily routines — strictly no gaming references). Enforced Four Pillars (Foundation mental models, Mechanism with proactive doubt-busting disarming common misconceptions, Application real-world grounding, and Knowledge Bridge to what connects next) and 1-sentence principle-focused quiz explanations.
  - **Unified Linear Onboarding & Single Sign-In Flow**: Consolidated all sign-in triggers to direct to `/onboarding`. Drafted questions in `QuestionInput` survive in `sessionStorage` into Step 5 for 1-tap start. Step 3 provides a 1-tap Google OAuth (`signIn.sso()`) authentication matching Clerk's OAuth-only strategy without modal popups or external breaks. `/sign-in` and `/sign-up` auto-redirect to `/onboarding`.
...
- 2026-08-22 (fix) — **Fix Lite Mode Answer / Part Card Visibility & Pre-Paint Theme Initialization.**
  - **Issue**: Selecting "Lite" performance mode (`data-perf="low"`) or Auto device-tier detection resolving to Lite dimmed unlocked lesson part cards on `/learn` to `opacity: 0.12`, making answer cards appear black/dark and unreadable. In addition, the pre-paint `themeInitScript` in `layout.tsx` failed to set `document.documentElement.dataset.theme = t` when `t === "light"`.
