...
LPU inference (Llama 3.3 70B / GPT-OSS 120B / Llama 3.1 8B, plain fetch — no
...
- 2026-08-18 — **MAJOR MILESTONE: Backend rebuilt around the new provider stack (aiEngine.js) — full AI-pipeline audit, latency overhaul & legacy purge.**
  ...
  - **Neutral user-facing failure copy**: no provider names/internals ("AI API request timed out…" → "This request timed out after N seconds. Please try again."), retryability phrases (`try again`/`temporarily`/`timed out`) preserved for the frontend classifier.
  - **Cleanup**: `AI_DISABLE_THINKING=groq` no-op mode removed (nvidia/cloudflare only), `GEMMA_*` env fallbacks dropped, stale `CEREBRAS_API_KEY` fixed to `GROQ_API_KEY` in `scripts-smoke/integration-smoke.sh`, `.env.example` rewritten to match.
  - **Verification**: backend suite rewritten as `test/ai-engine.test.js` (+6 new tests: Groq endpoint/payload contract, Mistral JSON-mode on/off, TPM affinity/rotation/429-poisoning, exact-usage accounting) — 95/95 passing. Live end-to-end harness (real server, mocked provider/Clerk network) verified: fast & explain streaming, provider-error fail-fast rescue, malformed-JSON repair rung, silent-stream hedge rescue (~1.7s), total-outage neutral error + half-open recovery, mid-generation client cancel with slot release, and 3-way concurrent single-flight dedupe.
