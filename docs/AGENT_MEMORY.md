...
- 2026-08-14 — **Chatbox UI Modernization, Mode Simplification & Clutter Removal**:
  ...
  - **Chat Box Redesign & Consistent Enter Action**: Redesigned `QuestionInput.tsx` into a lightweight, modern single-bar layout (`.q-form__bar`), moving mode glider and tools into unified left/right groupings. Added an explicit, high-contrast **Enter ↵** action button (`.q-form__enter-btn` / `.q-form__enter-symbol`) and natural `Enter` key form submission (with `Shift+Enter` for multiline).
