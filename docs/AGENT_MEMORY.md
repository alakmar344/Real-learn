...
aesthetic is the **RealLearn Brand DNA & Tactile Folio** design language — calm surface, warm core:
Solar Terracotta `#EE5125` (light) / `#FF6435` (dark) as the primary interactive accent,
Slate Cobalt `#1E293B` as grounding secondary, Sage Emerald `#04A16C` / `#00D284` for
success/correctness, Sunlight Alabaster Paper `#FAF9F6` by day and Midnight Obsidian Slate
`#0D1117` by night. Body text is Inter 400 (Space Grotesk 700-800 for headings, Lora for reading prose),
lesson prose never drops below 16px on phones, every color pairing is WCAG-AA-verified, and there is NO
neon and NO purple/violet (owner's rule). Two moods of the same room: Paper (day) and Ink (night).
JS-side colors (confetti, share card) come from `frontend/lib/palette.ts` — never hardcode brand hexes in components. Full
