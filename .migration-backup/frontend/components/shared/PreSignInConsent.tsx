...
const POLICY_CHANGES = [
  "Privacy Policy updated to version 2.8 — all users are re-prompted to re-accept.",
  "Clearer minors' consent wording: for learners aged 13–17 we ask for an explicit onboarding confirmation that a parent or guardian has reviewed and approved use of RealLearn (a self-attested confirmation — we do not run an independent parent identity-verification process).",
  "MongoDB Atlas is now listed in the Third-Party Services section as the database that stores account, preference, consent, and feedback records (encrypted in transit and at rest).",
...
const TERMS_CHANGES = [
  "Terms of Service updated to version 2.6: intellectual-property wording clarified (Sections 7 and 15) to claim only rights that exist under applicable law, and the feedback-prompt timing description now matches the product (soon after your first completed lesson).",
  "Version 2.5 added Section 23 describing optional learning personalization. Your learning-style preferences and free-text notes are stored only on your device, included with each lesson request for tailoring, and can be changed or removed anytime in Settings. We do not store this information on our servers.",
  ...
  "Optional feedback (Section 22): soon after your first completed lesson, RealLearn may invite an optional 1–10 star rating plus free-text notes. The review is entirely optional and never required to use the Service; it is submitted anonymously (no account identity, no IP/Clerk ID/email stored), and a local flag prevents the prompt from reappearing once you respond or decline.",
];
